...
This is a placeholder for the data-science framing of the web-fetch + schema-validated extraction loop. It mirrors `cc-writing-team/skills/web-research` (research framing) on the opposite side of the same primitive.

## What it will do

- Accept a list of URLs (or a query that produces them via WebSearch)
- Invoke the cc-os Layer-1 primitive (`cc-os/scripts/fetch-source.py`) for each
...
Reserved 2026-05-03 in JCC-731 (Bucket B+C capability extensions plan). Plan rationale: URL → markdown is the same primitive for writing-research and data-extraction, but the orchestrators are framed differently — research = quotes/credibility/sources; extraction = schema/validation/dataset rows. Two distinct skills, one shared cc-os primitive.

## How to invoke today

Until this is built, agents that need URL → structured data should:

1. Use the cc-os `web-fetch` skill directly:
   ```bash
   python3 ~/Documents/GitHub/jf-private/claude-setup/cc-os/scripts/fetch-source.py "<url>"
   ```
2. Run an LLM extraction pass against the resulting markdown using whatever schema fits the task (typically Pydantic + Gemini `gemini-3.1-pro-preview` per `cc-os/rules/environment-and-models.md`).
